...
export const PROVIDERS = {
  ...
  'korgun-mssql': {
    ...
    configSchema: [
      ...
      { key: 'port', label: 'Port', type: 'string', required: false, placeholder: 'boş = SQL Browser' },
      { key: 'database', label: 'Veritabanı', type: 'string', required: true, placeholder: 'KorgunDB' },
      { key: 'integratedSecurity', label: 'Windows Authentication (NTLM)', type: 'boolean', required: false, default: false },
      { key: 'user', label: 'Kullanıcı', type: 'string', required: false, placeholder: 'sa (SQL auth) veya domain\\user (NTLM)' },
      { key: 'password', label: 'Şifre', type: 'password', required: false, placeholder: '••••••' },
